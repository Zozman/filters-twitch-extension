...
export default class ExtensionConfig extends ExtensionBase {
    ...
    /**
     * Function to get the demo steps.  Done like this so locale is taken into account.
     * @returns The demo steps
     */
    private getDemoSteps() {
        return [
            {
            ...
            {
                'text': msg('Use the Customize menu to adjust your filter or make one from scratch.'),
    ...
    render() {
        return html`
            ${this.loadLocaleTask.render({
                complete: () => {
                    const demoSteps = this.getDemoSteps();
                    return html`
